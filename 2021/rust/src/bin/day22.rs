...
impl Solution {
    fn count_cubes(&self, steps: &[Step]) -> i64 {
        ...
        let hms = vs
            ...
            .collect::<Vec<_>>();
        let mut grid =
            vec![vec![vec![false; vs[2].len() * 2 + 1]; vs[1].len() * 2 + 1]; vs[0].len() * 2 + 1];
        for &(turn, (x, y, z)) in steps {
            #[allow(clippy::needless_range_loop)]
            for i in hms[0][&x.0]..=hms[0][&x.1] {
                for j in hms[1][&y.0]..=hms[1][&y.1] {
                    for k in hms[2][&z.0]..=hms[2][&z.1] {
                        grid[i][j][k] = turn;
                    }
                }
            }
        ...
        let mut ret = 0;
        #[allow(clippy::needless_range_loop)]
        for i in 0..=vs[0].len() * 2 {
            for j in 0..=vs[1].len() * 2 {
                for k in 0..=vs[2].len() * 2 {
                    if !grid[i][j][k] {
                        continue;
                    }
                    let mut on = 1;
                    if i & 1 > 0 {
                        on *= vs[0][i / 2 + 1] - vs[0][i / 2] - 1;
                    }
                    if j & 1 > 0 {
                        on *= vs[1][j / 2 + 1] - vs[1][j / 2] - 1;
                    }
                    if k & 1 > 0 {
                        on *= vs[2][k / 2 + 1] - vs[2][k / 2] - 1;
                    };
                    ret += on;
                }
            }
